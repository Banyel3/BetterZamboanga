/* Better Zamboanga - Enhanced Search Functionality (forked from BetterSolano.org) */
/* Updated: 2025-12-05 */
...
(function () {
  ...
  const ANALYTICS_KEY = "betterzamboanga_search_analytics";
  const RECENT_SEARCHES_KEY = "betterzamboanga_recent_searches";
  ...
  function searchServices(query, services, options = {}) {
    ...
    if (searchIndex) {
      searchTerms.forEach((term) => {
        ...
        findFuzzyMatches(term, searchIndex.titleIndex).forEach((idx) =>
        ...
        // Office matches
        findFuzzyMatches(term, searchIndex.officeIndex).forEach((idx) =>
          candidateIndices.add(idx)
        );
      });
    } else {
  ...
  function createAutocomplete(input) {
    const existingDropdown = input.parentElement.querySelector(
      ".search-autocomplete"
    );
  ...
  function renderResults(results, dropdown, options = {}) {
    ...
    if (
      showSuggestions &&
      (suggestions.popular?.length || suggestions.recent?.length)
    ) {
        ...
        html += `
                    <div class="search-section">
                        ...
                        ${suggestions.recent
                          .map(
                            (term) => `
                            <a href="#" class="search-suggestion-item" data-suggestion="${escapeHtml(
                              term
                            )}">
                                <i class="bi bi-arrow-counterclockwise"></i> ${escapeHtml(
                                  term
                                )}
                            </a>
                        `
      ...
      .map((result, index) => {
        ...
        return `
                <a href="${url}" class="search-result-item" role="option" data-index="${index}">
                    <div class="search-result-title">
                        ...
                        ${
                          ...
                          result.processingTime
                            .toLowerCase()
                            .includes("same day")
                            ? '<span class="search-result-badge">Fast</span>'
                            : ""
                        }
                    </div>
                    <div class="search-result-meta">
                        <span class="search-result-category"><i class="bi bi-folder"></i> ${escapeHtml(
                          result.category
                        )}</span>
                        ${
                          result.fee
                            ? `<span class="search-result-fee"><i class="bi bi-cash"></i> ${escapeHtml(
                                result.fee
                              )}</span>`
                            : ""
                        }
                        ${
                          result.processingTime
                            ? `<span class="search-result-time"><i class="bi bi-clock"></i> ${escapeHtml(
                                result.processingTime
                              )}</span>`
                            : ""
                        }
                    ...
                    ${
                      result.office
                        ? `<div class="search-result-office"><i class="bi bi-building"></i> ${escapeHtml(
                            result.office
                          )}</div>`
                        : ""
                    }
    ...
    html += `
            <div class="search-footer">
                <span class="search-footer-count">${results.length} service${
      results.length !== 1 ? "s" : ""
    } found</span>

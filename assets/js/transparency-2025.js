...
function updateMetrics(data) {
  ...
  const metricExpendituresSource = document.getElementById(
    "metric-expenditures-source"
  );
...
function updateIncomeSection(data) {
  ...
  updateElement(
    "income-local-pct",
    calculatePercentage(income.local.total, total) + "%"
  );
  updateElement(
    "income-tax-revenue",
    formatPesoMillions(income.local.taxRevenue.total)
  );
  updateElement(
    "income-rpt",
    formatPesoMillions(income.local.taxRevenue.realPropertyTax.total)
  );
  updateElement(
    "income-business-tax",
    formatPesoMillions(income.local.taxRevenue.businessTax)
  );
  updateElement(
    "income-non-tax",
    formatPesoMillions(income.local.nonTaxRevenue.total)
  );

  // Update external sources
  updateElement(
    "income-external-amount",
    formatPesoMillions(income.external.total)
  );
  updateElement(
    "income-external-pct",
    calculatePercentage(income.external.total, total) + "%"
  );
  updateElement(
    "income-nta",
    formatPesoMillions(income.external.nationalTaxAllotment)
  );
  updateElement(
    "income-other-shares",
    formatPesoMillions(income.external.otherShares)
  );
...
function updateExpenditureSection(data) {
  ...
  updateElement(
    "exp-gps-amount",
    formatPesoMillions(exp.generalPublicServices)
  );
  ...
  updateElement(
    "exp-social-amount",
    formatPesoMillions(exp.socialServices.total)
  );
  ...
  updateElement(
    "exp-welfare",
    formatPesoMillions(exp.socialServices.socialWelfare)
  );
  updateElement(
    "exp-education",
    formatPesoMillions(exp.socialServices.education)
  );

  // Update Economic Services
  updateElement(
    "exp-economic-amount",
    formatPesoMillions(exp.economicServices)
  );
...
function initScrollAnimations() {
  ...
  document
    .querySelectorAll(".animate-on-scroll, .metric-card")
    .forEach((el) => {
      observer.observe(el);
    });
}

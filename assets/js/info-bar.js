...
const CONFIG = {
  ...
  ZAMBOANGA_LAT: 6.9214,
  ZAMBOANGA_LON: 122.079,
...
const ExchangeRateService = {
  ...
  cacheRates(data) {
    CacheManager.set(
      CONFIG.CACHE_KEYS.EXCHANGE_RATES,
      data,
...
const WeatherService = {
  ...
  async fetchWeather() {
    try {
      const response = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${CONFIG.ZAMBOANGA_LAT}&longitude=${CONFIG.ZAMBOANGA_LON}&current_weather=true`
      );
...
const InfoBarManager = {
  ...
  renderCurrentRate() {
    ...
    if (!displayElement || !this.currentRates || !this.currentRates.rates)
      return;
  ...
  rotateRate() {
    this.currentRateIndex =
      (this.currentRateIndex + 1) % CONFIG.CURRENCIES.length;
    this.renderCurrentRate();
  },

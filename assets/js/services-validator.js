/**
 * Better Zamboanga - Services Data Validator (forked from BetterSolano.org)
...
function validateServiceEntry(entry) {
  ...
  if ("id" in entry && typeof entry.id === "string") {
    if (!/^[a-z0-9-]+$/.test(entry.id)) {
      warnings.push(
        `ID "${entry.id}" should be kebab-case (lowercase with hyphens)`
      );
    }
  }
...
function validateServicesData(data) {
  ...
  for (let i = 0; i < data.services.length; i++) {
    ...
    result.errors.forEach((e) =>
      errors.push(`Entry ${i} (${entry?.id || "unknown"}): ${e}`)
    );
    result.warnings.forEach((w) =>
      warnings.push(`Entry ${i} (${entry?.id || "unknown"}): ${w}`)
    );
  }

...
(function () {
  ...
  function getStatusBadge(status) {
    if (status === 100)
      return '<span class="dpwh-badge complete">Completed</span>';
  ...
  function renderSection(container, data) {
    ...
    const html = `
            <div class="dpwh-summary-bar">
                ...
                <div class="dpwh-summary-item">
                    <span class="dpwh-summary-value">₱${(
                      data.summary.totalCost / 1000000
                    ).toFixed(1)}M</span>
                ...
                <div class="dpwh-summary-item">
                    <span class="dpwh-summary-value">${
                      data.summary.totalProjects - completedCount
                    }</span>
            ...
            <div class="dpwh-controls">
                <div class="dpwh-filter-group" role="tablist" aria-label="Filter projects by category">
                    <button class="dpwh-tab active" data-filter="all" role="tab" aria-selected="true">All <span class="dpwh-tab-count">${
                      counts.all
                    }</span></button>
                    <button class="dpwh-tab" data-filter="buildings" role="tab" aria-selected="false">Buildings <span class="dpwh-tab-count">${
                      counts.buildings
                    }</span></button>
                    <button class="dpwh-tab" data-filter="roads" role="tab" aria-selected="false">Roads <span class="dpwh-tab-count">${
                      counts.roads
                    }</span></button>
                    <button class="dpwh-tab" data-filter="flood" role="tab" aria-selected="false">Flood Control <span class="dpwh-tab-count">${
                      counts.flood
                    }</span></button>
                    ${
                      counts.water > 0
                        ? `<button class="dpwh-tab" data-filter="water" role="tab" aria-selected="false">Water <span class="dpwh-tab-count">${counts.water}</span></button>`
                        : ""
                    }
                </div>
            </div>
                ...
                <tr class="dpwh-row" tabindex="0">
                    <td class="col-desc">
                        <div class="dpwh-desc-wrap">
                            ...
                            <span class="dpwh-cat-badge ${getCategoryClass(
                              p.category
                            )}">${getCategoryLabel(p.category)}</span>
                        </div>
                        <span class="dpwh-proj-title" title="${
                          p.name
                        }">${truncateText(p.name, CONFIG.truncateLength)}</span>
                        <span class="dpwh-proj-location"><i class="bi bi-geo-alt"></i>${
                          p.location
                        }</span>
                    </td>
                    <td class="col-contractor">
                        <span class="dpwh-contractor">${p.contractor}</span>
                        <span class="dpwh-contractor-id">#${
                          p.contractorId
                        }</span>
                    </td>
  ...
  function loadMoreRows() {
    ...
    isLoading = true;
    const rowsToLoad =
      displayedCount === 0 ? CONFIG.initialRows : CONFIG.loadMoreRows;
